...
void relayAction(int adrON, int adrOF, int pos, int pin) {
  rtc.refresh();
	int MinToday = (rtc.hour() * 60) + rtc.minute();
...
void setRTC() {
	...
	lcd.setCursor(0, 0);
	lcd.print("CURRENT ");
	setupShowValue(rtc.hour(), rtc.minute(), 0);

	lcd.setCursor(0, 1);
	lcd.print("NEW ");

  rtc.refresh();
	setupShowValue(rtc.hour(), rtc.minute(), 1);
	while (read_LCD_buttons() == btnNONE);
	setupChooseValueSetRTC(rtc.hour(), rtc.minute(), 1);
	lcd.setCursor(0,0);
...
void setupChooseValueSetRTC(int HourNew, int MinNew, byte Pos) {
  ...
  rtc.set(0, MinNew, HourNew, 6, 10, 1, 14);
